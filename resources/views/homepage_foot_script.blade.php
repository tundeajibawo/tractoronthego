...
<!-- Script to retrieve data fro the database -->
<!-- Script -->


<script type="text/javascript">

    $('#farm-profile').on('click', function(){
      alert('Working');
    })
        ...
        setInterval(function() {
            $('#location:not(.appeared)')
                ...
                .each(function() {
                            ...
                            processResults: function (data) {
                                return {
                                    results: $.map(data.suggestions, function (obj) {
                                        return { id: obj.label.split(', ').reverse().join(', '), text: obj.label.split(', ').reverse().join(', ') };
                                    })
                                };
                            }
                        },
                    ...
                    $('#location').select2(options).on("change", function(e) {
                        var farm_location = $('#location').val().replace(/<\/?[^>]+(>|$)/g, "");

                        // todo: retrieve ip address from farm_location
                    })
                });
        }, 250);
